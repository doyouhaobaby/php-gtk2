GtkVButtonBox 将一些按钮垂直地摆在一排。在应用程序中，按钮盒子能够提供一个排列按钮的容器。我们使用像其它容器一样的添加元件的方法add().pack_start和pack_end方法跟add()是一样的效果，根据当前布局样式添加元件和取决于子按钮是否使用调用set_child_secondary()。
